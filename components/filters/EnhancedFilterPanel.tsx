...
export function EnhancedFilterPanel() {
  ...
  // IMPORTANT: Volume data only exists for "By Product Type", so filter accordingly
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          {Object.keys(hierarchy).length === 0 && availableSegments.length === 0 ? (
            ...
            <>
              {/* DEBUG: Log hierarchy structure for By Country */}
              {selectedSegmentType === 'By Country' && (() => {
                console.log('🔍 CLIENT-SIDE: CascadeFilter receiving hierarchy for By Country:')
                ...
                console.log('  Root items (keys not in children):', roots)
                return null
              })()}
              <CascadeFilter
                hierarchy={hierarchy}
