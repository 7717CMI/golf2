...
export function GlobalKPICards() {
  ...
  const kpiData = useMemo(() => {
    ...
    // Get start and end years from metadata
    const startYear = data.metadata.start_year || 2024
    const forecastYear = data.metadata.forecast_year || 2033

    // Calculate total market size for start and forecast years
    let marketSizeStart = 0
    let marketSizeForecast = 0

    globalRecords.forEach(record => {
      marketSizeStart += record.time_series[startYear] || 0
      marketSizeForecast += record.time_series[forecastYear] || 0
    })

    // Calculate CAGR from start to forecast year
    const years = forecastYear - startYear
    const cagr = marketSizeStart > 0 && years > 0
      ? (Math.pow(marketSizeForecast / marketSizeStart, 1 / years) - 1) * 100
      : 0

    // Calculate absolute growth
    const absoluteGrowth = marketSizeForecast - marketSizeStart
    const growthPercentage = marketSizeStart > 0
      ? ((marketSizeForecast - marketSizeStart) / marketSizeStart) * 100
      : 0
    ...
    const marketSizeForecastDisplay = marketSizeForecast
    ...
    return {
      ...
      marketSizeForecast: marketSizeForecastDisplay,
      cagr,
      ...
      <div className="container mx-auto px-6 py-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-black uppercase tracking-wider font-semibold">
                {kpiData.dataTypeLabel} {kpiData.forecastYear}
              </p>
              <p className="text-base font-bold text-black leading-tight">
                {kpiData.dataType === 'value' && kpiData.isINR
                  ? `₹ ${formatIndianNumber(kpiData.marketSizeForecast)}`
                  : kpiData.dataType === 'value'
                  ? `$ ${kpiData.marketSizeForecast.toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} ${kpiData.unit}`
                  : `${kpiData.marketSizeForecast.toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} ${kpiData.unit}`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-black uppercase tracking-wider font-semibold">
                CAGR ({kpiData.startYear}-{kpiData.forecastYear})
              </p>
              <p className="text-base font-bold text-black leading-tight">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-black uppercase tracking-wider font-semibold">
                Absolute Growth ({kpiData.startYear}-{kpiData.forecastYear})
              </p>
              <p className="text-base font-bold text-black leading-tight">
                {kpiData.dataType === 'value' && kpiData.isINR
              ...
              <p className="text-[10px] text-gray-600 mt-0.5">
                {kpiData.growthPercentage >= 0 ? '+' : ''}{kpiData.growthPercentage.toFixed(1)}% {kpiData.growthPercentage >= 0 ? 'increase' : 'decrease'}
              </p>
            </div>
          </div>
        </div>
